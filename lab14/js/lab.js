...
function fizzBuzz(max, numInput, txtInput) {
  ...
  for (var num = 1; num <= max; num++) {
    ...
    if (outstring.length>0){
        console.log(num+ '-'+outstring); //Testing to see if the outputted string/number combo is correct printing it each time
        finalstring+=num+'-'+outstring+"!</br>";
    }
    //otherwise just output the raw number
    else{
      console.log(num);//Testing to see if the outputted string/number combo is correct printing it each time
      finalstring+=num+"</br>";
    }
}
